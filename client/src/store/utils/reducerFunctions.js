export const addMessageToStore = (state, payload) => {
  const { message, sender, isSenderCurUser } = payload;
  ...
  if (sender !== null) {
    const newConvo = {
      ...
      otherUser: sender,
      messages: [message],
      unreadCount: isSenderCurUser ? 0 : 1
    };
...
export const clearUnreadConvoFromStore = (state, conversationId) => {
  return state.map((convo) => {
...
export const receiveOtherHasReadToStore = (state, conversationId) => {
  return state.map(convo => {
    if (convo.id === conversationId) {
      ...
      if (lastReadByOtherMessage) {
        convoCopy.lastReadByOtherMessageId = lastReadByOtherMessage.id;
      } else {
        convoCopy.lastReadByOtherMessageId = null;
      }
      return convoCopy
    } else {
      return convo
